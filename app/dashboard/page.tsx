...
export default function DashboardPage() {
  ...
  return (
    <main className="dashboard-page">
      ...
      <aside className="dashboard-sidebar"><nav><a className="active" href="#overview"><span>⌁</span>Overview</a><a href="#quota"><span>◫</span>Quota package</a><a href="#activity"><span>↻</span>Activity</a><a href="#billing"><span>◇</span>Billing</a></nav><div className="sidebar-bottom"><span>PROJECT</span><strong>{shortId(account.user.openstack_project_id)}</strong><small>OpenStack tenant</small></div></aside>
      <section className="dashboard-content" id="overview">
        ...
        <div className="dashboard-grid">
          <article className="panel quota-panel" id="quota"><div className="panel-heading"><div><span>QUOTA PACKAGE</span><h2>{plan.name}</h2></div><small>Changes apply to your OpenStack project</small></div><div className="quota-options">{PLANS.map((option) => <button key={option.id} className={option.id === plan.id ? "quota-option quota-option--active" : "quota-option"} disabled={Boolean(status.changing) || option.id === plan.id} onClick={() => changePlan(option.id)}><span>{option.name}{option.id === plan.id && <i>CURRENT</i>}</span><strong>{option.vcpus} vCPU · {option.memoryGb} GB · {option.storageGb} GB</strong><small>{status.changing === option.id ? "Applying…" : option.id === plan.id ? "Active allocation" : "Switch package →"}</small></button>)}</div><p className="panel-note">Package changes are validated against host headroom before being applied. No charge is created during the POC.</p></article>
          <article className="panel activity-panel" id="activity"><div className="panel-heading"><div><span>RECENT ACTIVITY</span><h2>Audit trail</h2></div></div><div className="activity-list">{account.changes.results.map((change) => <div key={change.id}><i>↻</i><p><strong>{change.from_plan_id ? "Quota package updated" : "Tenant provisioned"}</strong><small>{change.from_plan_id ? `${title(change.from_plan_id)} → ${title(change.to_plan_id)}` : `${title(change.to_plan_id)} package applied`}</small></p><time>{formatDate(change.created_at)}</time></div>)}</div></article>
        </div>
