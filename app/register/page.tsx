...
const initialForm: FormState = { firstName: "", lastName: "", organisation: "", email: "", password: "", confirmPassword: "", planId: "scale", terms: false };
      ...
      <div className="auth-shell">
        ...
        <section className="auth-form-wrap">
          ...
          <form onSubmit={submit} className="registration-form">
            <fieldset><legend>Your details</legend><div className="field-grid"><Field label="First name" value={form.firstName} onChange={(value) => setField("firstName", value)} autoComplete="given-name" /><Field label="Last name" value={form.lastName} onChange={(value) => setField("lastName", value)} autoComplete="family-name" /><Field label="Organisation" value={form.organisation} onChange={(value) => setField("organisation", value)} autoComplete="organization" optional /><Field label="Email address" type="email" value={form.email} onChange={(value) => setField("email", value)} autoComplete="email" /></div></fieldset>
            <fieldset><legend>OpenStack credentials</legend><div className="field-grid"><Field label="Password" type="password" value={form.password} onChange={(value) => setField("password", value)} autoComplete="new-password" hint="10+ characters" /><Field label="Confirm password" type="password" value={form.confirmPassword} onChange={(value) => setField("confirmPassword", value)} autoComplete="new-password" /></div><p className="field-note"><span>↗</span> These credentials are passed once to OpenStack. BaseGrid stores your account, package and quota history—never the password.</p></fieldset>
            <fieldset><legend>Select your quota</legend><div className="register-plans">{PLANS.map((plan) => <label className={form.planId === plan.id ? "register-plan register-plan--selected" : "register-plan"} key={plan.id}><input type="radio" name="plan" value={plan.id} checked={form.planId === plan.id} onChange={() => setField("planId", plan.id)} /><div><span>{plan.name}</span>{plan.highlight && <small>RECOMMENDED</small>}</div><p>{plan.vcpus} vCPU · {plan.memoryGb} GB RAM · {plan.storageGb} GB</p><i>{form.planId === plan.id ? "●" : "○"}</i></label>)}</div></fieldset>
